...
app.post('/tyot', async (req, res) => {
  try {
    if (!req.files) {
      ...
      let lista = req.files.lista; //Use the mv() method to place the file in upload directory (i.e. "uploads")
      //avatar.mv('./uploads/' + lista.name);

      const file_data = lista.data; // all options are optional

      _pdfdataextract.PdfData.extract(file_data, {
        pages: 1,
        // how many pages should be read at most
        sort: true,
        // sort the text by text coordinates
        verbosity: _pdfdataextract.VerbosityLevel.ERRORS,
        ...
        let {
          value
        } = ics.createEvents(moi);
        (0, _fs.writeFileSync)('public/events.ics', value);
        res.sendFile('events.ics', {
          root: 'public'
        });
      }); //send response
...
function parse(text) {
  ...
  let matches;
  let events = [];
  if (helsinki.test(text)) matches = text.matchAll(helsinki);else if (pori.test(text)) matches = text.matchAll(pori);else return "Error";
  let listobj = [];
